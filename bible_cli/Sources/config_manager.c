...
int initConfig(ConfigParams *vals, const char *configPath) {
  if (!vals || !configPath || strlen(configPath) == 0) {
    return makeError(
        "Your config path and config parameters must be real values.");
  }
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    if (strlen(line) == 0 || line[0] == '*')
      continue; // * is a symbol for comments
    char *delimiter = strchr(line, '=');
    if (delimiter == NULL) {
      makeError("Invalid line is found in bible.conf. For comments you should "
    ...
    *delimiter = 0;
    ...
    if (strlen(key) == 0 || strlen(val) == 0) {
      makeError(
          "Error: Empty key or value. Check the file bible.conf for errors.");
      continue;
    }
    // VALUES
